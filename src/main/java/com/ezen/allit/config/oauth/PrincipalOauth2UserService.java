...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws  OAuth2AuthenticationException {
		...
		String providerId = oAuth2UserInfo.getProviderId(); // 1231231231242512241(임의의 난수)
		String username = provider + "_" + providerId;
		String password = encoder.encode("1234");
		String email = oAuth2UserInfo.getEmail(); // google_12321421521412421
		Role role = Role.MEMBER;
		Grade grade = Grade.BRONZE;
		
		Member member = memberRepo.findById(username).orElse(null);
		if(member == null) { // 기존 가입기록이 없으면 회원가입 진행
